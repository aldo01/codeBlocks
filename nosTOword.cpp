...
char *digits[10] = {"", " One "," Two ",
                    " Three "," Four ",
                    " Five "," Six ",
                    " Seven "," Eight "," Nine "};
char *all_ens[10] = { " ten "," eleven "," twelve "," thirteen ",
                      " fourteen "," fifteen ", " sixteen ",
                      " seventeen "," eighteen "," nineteen " };

char *all_tys[8] = { "twenty","thirty","forty","fifty",
...
void show(int low,int high)
{
    ...
    {
        //printf("(");
        show(low,high - BASE_NUM_LEN);
       // printf(")");
        printf(" Crore ");
        show(high-BASE_NUM_LEN+1,high);
    }
    else{
        switch(len)
       {
...
int print1(int low)
{
    if(number[low] == '0')
        return 0;
    printf(digits[number[low]-ASCII_OF_ZERO]);
    return 1;
}/*end of print1()*/
...
int print2(int low, int high)
{
    ...
    case '0':
             if(print1(high))
                ...
                return 0;
    case '1':
                printf(all_ens[number[high] - ASCII_OF_ZERO]);
                break;
    default:
                /* this portion used for printing numbers
                 * from 20 to 99 i.e number[low] has value '2' -to- '9'*/
               print_20_99(all_tys[number[low] - ASCII_OF_ZERO-2],low,high); /* all_tys[] stores twenty in 0th loc */
               break;
     }
     return 1;
}/* end of print2()*/
